...
async function updatePlanes() {
    ...
    // Vamos tentar o pedido DIRETO primeiro. 
    // Com conta, o OpenSky às vezes é mais permissivo com o CORS.
    try {
        statusEl.innerText = "A autenticar faginea...";
        ...
        if (!response.ok) {
            // Se o direto falhar (CORS), tentamos via Proxy
            throw new Error("CORS block");
        }

        const data = await response.json();
        processPlanes(data.states || []);

    } catch (err) {
        // Se falhar, tentamos via Proxy (mas o proxy não envia a tua conta, infelizmente)
...
function processPlanes(planes) {
    ...
    planes.forEach(s => {
        // ... resto da lógica dos markers que já tinhas ...
        const icao = s[0];
